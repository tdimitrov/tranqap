...
rpcap looks for a file named config.json in the current working dir. A
...
This is the rpcap prompt. **targets** command checks if tcpdump can be
...
-  Last three lines are meaningful only if rpcap should run tcpdump with
